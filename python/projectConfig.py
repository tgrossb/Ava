...
def makeProjectConfigFile(loc):
	utils.out(utils.LINE_H, "ava: ", utils.STD_OUT, "Creating project configuration file at ", loc)
...
def readProjectConfigs(loc):
	...
	if utils.PROJECT not in config.sections():
		utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project configuration file missing section '" + utils.PROJECT + "'")
		utils.exit()

	# Look for project home first so @ can be define
	at = "."
	if utils.HOME not in config.options(utils.PROJECT):
		utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project configuration file missing parameter '" + utils.HOME + "'")
		utils.exit()
	else:
		at = config.get(utils.PROJECT, utils.HOME)
		if utils.HOME_SYM in at:
			utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project home parameter contains home symbol (" + utils.HOME_SYM + ")")
			utils.exit()
	...
	for param, value in utils.PROJECT_DEFAULTS[utils.PROJECT].items():
		...
		if param not in config.options(utils.PROJECT):
			utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project configuration file missing parameter '" + param + "'")
			utils.exit();
		...
		if utils.ALLOW_MULTIPLE_VALUES[param]:
			...
			for c in range(len(userVal)):
				...
				utils.out(utils.LINE_H, "ava: ", utils.STD_OUT, "Adding " + userVal[c] + " as a value for parameter '" + param + "'")
		elif "\n" in userVal:
			utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project configuration file includes multiple values for single value parameter '" + param + "'")
			utils.exit()
		else:
			userVal = utils.replaceSymbol(utils.HOME_SYM, at, userVal)
			utils.out(utils.LINE_H, "ava: ", utils.STD_OUT, "Recognized " + userVal + " as value for parameter '" + param + "'")
...
def findProjectConfigFile(name):
	...
	while not os.path.isfile(configLocation + '/' + name):
		if configLocation == '/':
			utils.out(utils.LINE_H, "ava: ", utils.ERR, "Project configuration file (", name, ") not found in parent directory")
			utils.out(utils.LINE_H, "ava: ", utils.ERR, "Run with ", utils.CMD, "--make-project-config", utils.ERR, " or ", utils.CMD, "-m", utils.ERR, " to create a project configuration file")
			utils.exit()
